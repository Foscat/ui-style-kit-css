...
import { syncDemoAssetVersions } from './sync-demo-asset-versions.mjs';
...
/**
 * Write the demo's runtime manifest snapshot from the package manifest.
 * @returns {Promise<void>} Resolves after the generated manifest is written.
 */
async function syncDemoManifest() {
  const demoManifest = {
  ...
  await writeGeneratedFile(path.join(root, 'demo', 'demo-manifest.js'), output);
}
...
async function syncOrganicIcons() {
  const iconDir = path.join(root, 'demo', 'assets', 'phosphor');
  const icons = Object.fromEntries(fs.readdirSync(iconDir).filter((name) => name.endsWith('.svg')).sort()
    .map((name) => [name.slice(0, -4), fs.readFileSync(path.join(iconDir, name), 'utf8')]));
  await writeGeneratedFile(
    path.join(root, 'demo', 'demo-organic-icons.js'),
    `/* Generated by scripts/build.mjs from licensed Phosphor icons in assets/phosphor. */\nwindow.ORGANIC_PHOSPHOR_ICONS = ${JSON.stringify(icons, null, 2)};\n`
  );
}

/**
 * Rebases bundled raster references for focused stylesheets one directory deeper.
 * @param {string} css Authored bundle.
 * @returns {string} CSS with portable focused-entrypoint URLs.
 */
function rebaseFocusedAssets(css) {
  if (!css.includes('./assets/')) return css;
  const ast = parse(css, { parseCustomProperty: true });
  walk(ast, (node) => {
    if (node.type === 'Url' && node.value.startsWith('./assets/')) {
      node.value = `../assets/${node.value.slice('./assets/'.length)}`;
    }
  });
  return generate(ast);
}
...
fs.cpSync(path.join(root, 'styles', 'assets'), path.join(dist, 'assets'), { recursive: true });
...
for (const preset of presetFiles) {
  const focusedBundle = rebaseFocusedAssets(bundle(visualSections([preset])));
  await writeGeneratedFile(path.join(focusedDist, `${preset.id}.css`), focusedBundle);
}

await syncDemoManifest();
await syncDemoIcons();
await syncOrganicIcons();
await syncReadmeBundleSizes(root);
await syncDemoAssetVersions(root);
