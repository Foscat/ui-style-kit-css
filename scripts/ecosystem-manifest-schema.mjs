...
export function validateEcosystemCompatibility(contract) {
  ...
  if (contract.ownership?.repository !== 'ui-style-kit-css' || contract.ownership?.status !== 'temporary' || typeof contract.ownership?.note !== 'string' || contract.ownership.note.trim().length === 0) throw new TypeError('Ecosystem compatibility ownership is invalid.');
  ...
  if (contract.packages.length !== packageNames.size || packageNames.size !== requiredPackages.size || [...requiredPackages].some((name) => !packageNames.has(name))) throw new TypeError('Ecosystem compatibility package identity is invalid.');
  ...
  for (const definition of contract.packages) {
    if (!definition || typeof definition.owns !== 'string' || definition.owns.trim().length === 0 || !/^>=\d+\.\d+\.\d+ <\d+\.0\.0$/.test(definition.supportedRange)) throw new TypeError('Ecosystem compatibility package ownership or supportedRange is invalid.');
  ...
  const canonicalSpecifiers = new Set(contract.canonicalImports.map((entry) => entry.specifier));
  if (canonicalSpecifiers.size !== contract.canonicalImports.length) throw new TypeError('Ecosystem canonical imports must be unique.');
  ...
  for (const entry of contract.deprecatedImports) if (typeof entry?.specifier !== 'string' || !entry.specifier.startsWith('ui-style-kit-css/') || entry.status !== 'deprecated' || typeof entry.replacement !== 'string' || !canonicalSpecifiers.has(entry.replacement)) throw new TypeError('Ecosystem deprecated import is invalid.');
  const deprecatedSpecifiers = new Set(contract.deprecatedImports.map((entry) => entry.specifier));
  if (deprecatedSpecifiers.size !== contract.deprecatedImports.length || !deprecatedSpecifiers.has('ui-style-kit-css/interactive-surface-bridge.css') || !deprecatedSpecifiers.has('ui-style-kit-css/with-bridge.css')) throw new TypeError('Ecosystem deprecated imports are incomplete.');
}
