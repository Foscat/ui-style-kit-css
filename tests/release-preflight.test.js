...
test('excludes only the selected Layout 3.2.3 candidate while retaining published minimum checks', async () => {
  assert.ok(releasePreflight, 'scripts/release-preflight.mjs must implement the release gate');
...
test('Interactive candidate preflight excludes only selected 1.7.3 while checking every published floor and companion', async () => {
  assert.ok(releasePreflight, 'scripts/release-preflight.mjs must implement the release gate');
...
test('still rejects a nonexistent companion current version for an unpublished candidate', async () => {
  ...
  const server = createServer((request, response) => {
    const [, packageName, version] = request.url.split('/');
    response.setHeader('content-type', 'application/json');
    if (decodeURIComponent(packageName) === 'interactive-surface-css' && version === '1.7.3') {
